...
const getQueryParams = async () => {
  ...
  urldata.value.callback_url = queryParams.callback_url;
  urldata.value.return_url = queryParams.return_url;
  ...
  try {
    const response = await axios.get('http://localhost:8000/pay/', {
      params: {
        ...
        callback_url: urldata.value.callback_url,
        return_url: urldata.value.return_url
      }
    });
